...
{
    public partial class Game : Canvas3D
    {
        ...
        private void ControlBtn_Click(object sender, RoutedEventArgs e)
        {
            ...
            switch (s)
            {
                ...
                case "L":
                    ...
                    break;
                case "R":
                    MoveRight(TestObject);
                    break;
                case "F":
                    MoveForward(TestObject);
                    break;
                case "T":
                    MoveTop(TestObject);
                    break;
                case "B":
                    MoveBottom(TestObject);
                    break;
                case "Y":
                    RotateByYAxis(TestObject);
                    break;
                case "X":
                    RotateByXAxis(TestObject);
                    break;
            }
        }

        private void MoveLeft(UIElement3D element)
        {
            var x = GetX(element);
            SetX(element, x - Step);
        }

        private void MoveRight(UIElement3D element)
        {
            var x = GetX(element);
            SetX(element, x + Step);
        }

        private void MoveForward(UIElement3D element)
        {
            var z = GetZ(element);
            SetZ(element, z - Step);
        }

        private void MoveBack(UIElement3D element)
        {
            var z = GetZ(element);
            SetZ(element, z + Step);
        }

        private void MoveTop(UIElement3D element)
        {
            var y = GetY(element);
            SetY(element, y + Step);
        }

        private void MoveBottom(UIElement3D element)
        {
            var y = GetY(element);
            SetY(element, y - Step);
        }

        private void RotateByYAxis(UIElement3D element)
        {
            var currentRotation = element.Rotation;
            element.Rotation = new Point3D(currentRotation.X, currentRotation.Y + Step*100*6, currentRotation.Z);
        }

        private void RotateByXAxis(UIElement3D element)
        {
            var currentRotation = element.Rotation;
            element.Rotation = new Point3D(currentRotation.X + Step*100*6, currentRotation.Y, currentRotation.Z);
        }
